...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Arul Lakshmi");
MODULE_DESCRIPTION("A simple Char DD to send and receive messages from user");
...
static ssize_t mychardev_read(struct file *file, char __user *buf, size_t count, loff_t *offset)
{
    uint8_t *data = "Hello and this is a message from the inserted char DD!\n";
...
static ssize_t mychardev_write(struct file *file, const char __user *buf, size_t count, loff_t *offset)
{
    ...
    if (ncopied == 0) {
        printk("Copied upto %zd bytes from the user\n", maxdatalen);

    } else {
        printk("Failed to copy %zd bytes from the user\n", ncopied);
    }

    databuf[maxdatalen] = 0;

    printk("Data sent from the user: %s\n", databuf);
